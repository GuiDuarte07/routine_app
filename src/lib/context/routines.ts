...
import {
  findConflictWithOccurrence,
  formatHourMinute,
  newEventHasTimeConflict,
  parseHourMinute,
  performTimeOperation,
  resolveTimeConflict,
  routineHasOccurrenceConflict,
} from '@/utils/routine';
...
export const useRoutine = create<RoutineStates & RoutineActions>(
  (set, get) => ({
    ...
    addNewEvent: (event) => {
      if (newEventHasTimeConflict(event, get().routine)) {
        throw ErrorRoutine(
          'CONFLICT',
          'Este horário já está ocupado por outro evento',
        );
      }
      set((state) => ({
      ...
      set((state) => {
        const updatedRoutine = state.routine.map((event) => {
          ...
          if (event.occurrence.some((occurrence) => occurrence.id === occId)) {
            ...
            return {
              ...event,
              occurrence: event.occurrence.filter(
                (occurrence) => occurrence.id !== occId,
              ),
            };
          }
    ...
    moveEventByKey: (arrowCode, shift, occurrenceId) => {
      set((state) => {
        ...
        const newEvent = routine.find((event) =>
          event.occurrence.find((occ) => occ.id === occurrenceId),
        );
        if (!newEvent)
          throw ErrorRoutine(
            'NOTFOUND',
            'Id dessa ocorrência não foi encontrado',
          );
        const occurence = newEvent?.occurrence.find(
        ...
        const moveOccurrenceByMinutes = (
          minutes: 30 | 60,
          operation: '+' | '-',
        ) => {
          const startHour = performTimeOperation(
            occurence.startHour,
            minutes,
            operation,
            true,
          );
          const endHour = performTimeOperation(
            occurence.endHour,
            minutes,
            operation,
            true,
          ...
          if (
            endHour !== occurence.endHour &&
            startHour !== occurence.startHour
          ) {
        ...
        const moveToNextWeekDay = (procediment: 'prev' | 'next') => {
          ...
          const dayPosition = arrayWeekDays.findIndex(
            (day) => day === actWeekDay,
          );
        ...
        switch (arrowCode) {
          case 'ArrowDown':
            direction = 'future';
            shift
              ? moveOccurrenceByMinutes(60, '+')
              : moveOccurrenceByMinutes(30, '+');
            break;
          case 'ArrowUp':
            direction = 'past';
            shift
        ...
        const conflitedOccId = findConflictWithOccurrence(
          occurence.id,
          routine,
        );
        if (conflitedOccId) {
          const { resolved, events } = resolveTimeConflict(
            occurence.id,
            routine,
            direction,
            shift ? 60 : 30,
